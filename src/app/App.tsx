...
const NAV_ITEMS = [
  ...
  { label: "Extracurricular",   href: "#certifications" },
...
const SKILLS = [
  ...
  { name: "Java",          level: 78, cat: "Backend"   },
  { name: "Node.js",         level: 75, cat: "Backend"   },
  { name: "SQL",         level: 70, cat: "Backend"   },
...
const PROJECTS = [
  {
    title: "Civiclens",
    desc: "Real-time civic issue reporting platform with geolocation, image uploads, and community voting. Built for local governments to streamline citizen feedback.",
    tags: ["Dart", "WebSockets", "Gemini API", "Node.js"],
    accent: "#3E7EC5",
    emoji: "🖼",
  },
  {
...
const EXPERIENCE = [
  {
    ...
    points: [
      "Worked on the Indore 311 civic platform, building AI-driven and citizen-focused features inspired by my CivicLens project to improve complaint validation, transparency, and public engagement.",
    ],
  },
  {
    role:    "Cloud Facilitator",
    company: "Cloud Study Jam by Google Developer Groups On Campus, DAVV",
    period:  "2025",
    points: [
      "Mentored 50+ students in completing Google Cloud pathways and labs.",
"Facilitated hands-on sessions on Cloud Computing, Virtualization, and Skill Boost profiles.",
...
const ACHIEVEMENTS = [
  { title: "State Chess Player",    year: "2019", icon: "🏆" },
  { title: "Speech Contest Winner",                      year: "2023", icon: "⚡" },
  { title: "Syncup 2026 TOP 5 finalists",    year: "2026", icon: "🥇" },
  { title: "Get Felicitation in WTM Event",      year: "2026", icon: "🎨" },
  { title: "Winning 2nd & 3rd Prize At APL",      year: "2026", icon: "🏅" },
];

const CERTS = [
  { name: "Marketing Lead",  issuer: "Google Developer Groups on Campus , DAVV",   color: "#4285F4" },
  { name: "Speaker Coordination & Social Media Marketing & Outreach Team",           issuer: "Google Developer Groups , INDORE", color: "#0082FB" },
  { name: "Marketing & Outreach Lead",            issuer: "WTM Break The Pattern",          color: "#FF9900" },
  { name: "Management Lead ",       issuer: "CEAC CLUB",color: "#34A853" },
];
...
function FloatingNav({ active, visible }: { active: number; visible: boolean }) {
  ...
  return (
    <header
      className="fixed top-4 left-[65%] -translate-x-1/2 z-50 transition-all duration-500"
      style={{ opacity: visible ? 1 : 0, transform: `translateX(-50%) translateY(${visible ? 0 : -16}px)` }}
    >
      {/* Desktop nav — पुराने साइज और पैडिंग के साथ */}
      <nav
      ...
      >
        {/* Anmol Brand Tag — पुराने फॉन्ट साइज और अलाइनमेंट के साथ सिंक किया गया */}
        <span
          className="px-2.5 py-1 text-sm font-normal whitespace-nowrap"
          style={{
            fontFamily: "var(--font-display)",
            color: "#1A1A2E",
            fontStyle: "italic",
            borderRight: "1px solid rgba(62,126,197,0.15)",
            marginRight: "4px",
            paddingRight: "12px",
          }}
        >
        ...
        {NAV_ITEMS.map((item, i) => {
          ...
          return (
            <a
              key={item.href}
              href={item.href}
              className="nav-link relative transition-all duration-200 flex flex-col items-center gap-0.5 whitespace-nowrap flex-shrink-0 px-2.5 py-1 rounded-xl"
              style={{ 
                color: isActive ? "#3E7EC5" : "#4A5568", 
                textDecoration: "none",
                fontSize: "0.75rem" // पुराना text-xs / 12px फॉन्ट साइज
              }}
              onMouseEnter={e => {
                e.currentTarget.style.background = "rgba(62,126,197,0.08)";
                e.currentTarget.style.color = "#3E7EC5";
                e.currentTarget.style.transform = "scale(1.05)";
              }}
              onMouseLeave={e => {
                e.currentTarget.style.background = "transparent";
                e.currentTarget.style.color = isActive ? "#3E7EC5" : "#4A5568";
            ...
            >
              ...
              {isActive && (
                <span
                  ...
                  style={{ width: 12, height: 1.5, background: "#3E7EC5" }} // पुरानी एक्टिव बार साइज (12px चौड़ी, 1.5px मोटी)
                />
              )}
            </a>
          );
      ...
      {/* Mobile nav — पुराने कॉम्पैक्ट साइज के साथ */}
      <div className="md:hidden">
        <button
          onClick={() => setOpen(!open)}
          className="flex items-center justify-center w-11 h-11 rounded-2xl" // पुराना 44px x 44px परफेक्ट साइज
          style={{
        ...
        {open && (
          <div
            className="absolute top-14 right-0 flex flex-col rounded-2xl p-3 gap-1 min-w-[160px]" // पुराना पैडिंग और मिनिमम विड्थ
            style={{
          ...
          >
            {NAV_ITEMS.map(item => (
              <a
                ...
                className="px-4 py-2.5 rounded-xl text-sm font-medium transition-all duration-150" // पुरानी लिंक्स पैडिंग
                style={{ color: "#1A1A2E" }}
...
function HeroSection() {
  ...
  return (
    ...
    >
      ...
      {/* Content */}
<div className="relative z-10 w-full max-w-5xl mx-auto px-6 sm:px-12 flex flex-col-reverse md:flex-row items-center justify-center gap-12 md:gap-20 pt-12 pb-24">
        {/* ── LEFT: Text column ── */}
        {/* Jo pehle change kiya tha use hatakar ab ye class laga dijiye: */}
<div
  className="flex-1 flex flex-col text-center md:text-left md:ml-20 md:mr-4 max-w-md"
...
>
          ...
          {/* Title */}
          <p className="text-base sm:text-lg md:text-xl font-light leading-relaxed max-w-sm mx-auto md:mx-0"
          ...
          {/* Quote */}
          <p
            className="text-sm sm:text-[15px] leading-[1.85] max-w-[22rem] mx-auto md:mx-0"
            style={{
              color: "#5A6880",
              fontFamily: "var(--font-display)",
              fontStyle: "italic",
      ...
      <div className="max-w-5xl mx-auto px-5 sm:px-8 md:px-12">
        ...
        <div className="grid sm:grid-cols-2 gap-8 items-start">
          <Reveal delay={100}>
            ...
            >
              <p className="leading-relaxed mb-5 text-[15px]" style={{ color: "#4A5568" }}>
                I&apos;m a third-year Information Technology student with an insatiable curiosity for
          ...
          <div className="flex flex-col gap-5">
            {[
              { label: "Currently Studying", value: "B.Tech Information Technology 3rd Year", icon: "🎓" },
              { label: "Interests",          value: "Web Dev · AI/ML · Data Analytics· Design", icon: "✦" },
...
// ─────────────────────────────────────────────────────────────────────────────
// CONTACT SECTION — bird-themed email
// ─────────────────────────────────────────────────────────────────────────────
...
function FinalSection() {
  ...
  // Smooth scroll handler to return to the top smoothly
  const handleReturnToBeginning = (e: React.MouseEvent<HTMLButtonElement>) => {
    e.preventDefault();
    window.scrollTo({
      top: 0,
      behavior: "smooth"
  ...
  return (
    <section id="contact" ref={sectionRef} className="relative pb-0 pt-24 md:pt-40 overflow-hidden text-center">
      {/* Atmospheric clouds */}
      ...
      <div className="relative z-10 max-w-2xl mx-auto px-5 sm:px-8">
        ...
        <Reveal delay={360}>
          <p className="text-[11px] tracking-[0.28em] uppercase mb-14" style={{ color: "#5A6880" }}>
        ...
        {/* REPLACED: Resume pill swapped with "Return to beginning" interactive button */}
        <Reveal delay={280}>
          <div className="flex justify-center mb-14">
            <button
              onClick={handleReturnToBeginning}
              className="connect-card relative flex items-center gap-2.5 select-none overflow-hidden text-white font-semibold text-[12px] tracking-wide"
              style={{
                padding: "10px 22px",
                borderRadius: "1rem",
                background: "linear-gradient(125deg, #3E7EC5 0%, #6B5FD8 55%, #E8960A 100%)",
                backgroundSize: "220% auto",
                border: "1px solid rgba(255,255,255,0.3)",
                boxShadow: "0 6px 24px rgba(62,126,197,0.35)",
                transition: "transform 0.22s cubic-bezier(.22,1,.36,1), box-shadow 0.22s ease",
              }}
              onMouseEnter={e => {
                e.currentTarget.style.transform = "translateY(-4px) scale(1.02)";
                e.currentTarget.style.boxShadow = "0 14px 32px rgba(62,126,197,0.45)";
              }}
              onMouseLeave={e => {
                e.currentTarget.style.transform = "translateY(0) scale(1)";
                e.currentTarget.style.boxShadow = "0 6px 24px rgba(62,126,197,0.35)";
              }}
            >
        ...
        <Reveal delay={360}>
          <p
            ...
            style={{ color: "#304f94", fontFamily: "var(--font-display)", fontStyle: "italic" }}
          >
      ...
      <footer className="relative z-10 w-full border-t py-5" style={{ borderColor: "rgba(62,126,197,0.1)", background: "rgba(255,255,255,0.22)", backdropFilter: "blur(12px)" }}>
        <p className="text-[11px] tracking-widest uppercase text-center" style={{ color: "#111216" }}>
          Designed &amp; built with care by&nbsp;
          <span style={{ color: "#6B7A9A", fontFamily: "var(--font-display)", fontStyle: "italic", fontSize: "13px", letterSpacing: "0.02em" }}>Anmol</span>
          &nbsp;
        </p>
      </footer>
    </section>
